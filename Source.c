/*Cole Stanley created this program after three hours and a lot of reading. This was for Professor Sanika Paranjape's Introduction to Programming Class on 09/11/2021 and 09/12/2021.
* This is for Assignment number three.
* The assignment is to create a program that allows the user to input any signed integer and receive, as the program output, the smallest input entered.
*/
#include <stdio.h>
#include <stdlib.h>
...
main() {
	...
	scanf_s("%I64i", &v2);
		if (min > v2) //Next, compare min to v2. If v2 is less than v1, store v2.
	{
		min = v2;
	}
	...
	scanf_s("%I64i", &v3);
		if (min > v1 || min > v2 || min > v3) //Last, compare v3 to v1, and v2. If v3 is smaller than v1 or v2, store v3. No else statement needed as the compare operation was done in previous statement and it is not changed unless v3 is smaller than v1 or v2. 
	{
		min = v3;
	}
	printf("Here's what you entered: %I64i\n", v3);
